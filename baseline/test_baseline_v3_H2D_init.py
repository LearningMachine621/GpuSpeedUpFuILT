...
def _cpu_init_single_tile(target_cpu: torch.Tensor) -> Tuple[np.ndarray, np.ndarray]:
    """完全在 CPU 上计算初始 target/mask 和 signed distance field。"""
...
def _async_save_single_tile(p_cpu: torch.Tensor, i: int, out_dir: str) -> None:
    """后台线程专用：只处理 CPU 张量的转换和保存，不访问 GPU。"""
...
def _async_save_large_mask(large_mask_cpu: torch.Tensor, large_mask_path: str) -> None:
    """后台线程专用：保存拼接后的大图。"""
    large_mask_img = (large_mask_cpu.numpy() > 0.5).astype(np.uint8) * 255
...
def gradient_fusion_split_optimization_tiles_v2(
    ...
    # CPU 结果统一搬运到 GPU（尽量利用 non_blocking + pinned memory）
...
if __name__ == "__main__":
    main()
'''
python -m baseline.test_baseline_v3_H2D_init --mode pytorch_fft
python -m baseline.test_baseline_v3_H2D_init --mode cuda_op
'''
