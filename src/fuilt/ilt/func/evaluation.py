...
ILT_DIR = Path(__file__).resolve().parent.parent
PROJECT_ROOT = Path(__file__).resolve().parents[4]
CONFIG_DIR = PROJECT_ROOT / "config"
DEFAULT_LITHO_CONFIG = CONFIG_DIR / "lithosimple.txt"
THIRDPARTY_ADAPTIVE_BOXES = ILT_DIR / "thirdparty" / "adaptive-boxes"

if THIRDPARTY_ADAPTIVE_BOXES.exists() and str(THIRDPARTY_ADAPTIVE_BOXES) not in sys.path:
    sys.path.append(str(THIRDPARTY_ADAPTIVE_BOXES))

try:
    from .settings import *
    from . import utils as common
    from . import exact as lithosim
    from . import proc, tools
except ImportError:
    from settings import *
    import utils as common
    import exact as lithosim
    import proc, tools


def _default_litho_model():
    return lithosim.LithoSim(str(DEFAULT_LITHO_CONFIG))

class Basic: 
    def __init__(self, litho=None, thresh=0.5, device=DEVICE): 
        self._litho  = litho if litho is not None else _default_litho_model()
        self._thresh = thresh
        self._device = device
...
def check(mask, sample, target, direction):
    # 初始化返回值，避免 UnboundLocalError
    inner = torch.empty((0, 2), dtype=sample.dtype, device=sample.device)
    outer = torch.empty((0, 2), dtype=sample.dtype, device=sample.device)
    
    # 边界检查
    if sample.shape[0] == 0:
        return inner, outer
    
    max_row = target.shape[0] - 1
    max_col = target.shape[1] - 1
    ...
    if direction == 'v':
        # 垂直方向检查
        if col + 1 <= max_col and col - 1 >= 0:
            if (target[row, col + 1] == 1) and (target[row, col - 1] == 0):
                # left, x small
                ...
                # 边界检查 inner 和 outer 坐标
                valid_inner = (inner[:, 0] >= 0) & (inner[:, 0] <= max_row) & (inner[:, 1] >= 0) & (inner[:, 1] <= max_col)
                valid_outer = (outer[:, 0] >= 0) & (outer[:, 0] <= max_row) & (outer[:, 1] >= 0) & (outer[:, 1] <= max_col)
                
                inner = inner[valid_inner]
                outer = outer[valid_outer]
                
                if inner.shape[0] > 0:
                    inner = inner[mask[inner[:, 0].long(), inner[:, 1].long()] == 0, :]
                else:
                    inner = torch.empty((0, 2), dtype=sample.dtype, device=sample.device)
                    
                if outer.shape[0] > 0:
                    outer = outer[mask[outer[:, 0].long(), outer[:, 1].long()] == 1, :]
                else:
                    outer = torch.empty((0, 2), dtype=sample.dtype, device=sample.device)

            elif (target[row, col + 1] == 0) and (target[row, col - 1] == 1):
                # right, x large  
                inner = sample + torch.tensor([0, -EPE_CONSTRAINT], dtype=sample.dtype, device=sample.device)
                outer = sample + torch.tensor([0, EPE_CONSTRAINT], dtype=sample.dtype, device=sample.device)
        ...
        # 水平方向检查
        if row + 1 <= max_row and row - 1 >= 0:
            if (target[row + 1, col] == 1) and (target[row - 1, col] == 0):
                # up, y small
            ...
            elif (target[row + 1, col] == 0) and (target[row - 1, col] == 1):
                # down, y large
                inner = sample + torch.tensor([-EPE_CONSTRAINT, 0], dtype=sample.dtype, device=sample.device)
                outer = sample + torch.tensor([EPE_CONSTRAINT, 0], dtype=sample.dtype, device=sample.device)
...
def evaluate(mask, target, litho, scale=1, shots=False, verbose=False):
    ...
    with torch.no_grad():
        ...
        if scale != 1:
            mask_bin = torch.nn.functional.interpolate(mask_bin[None, None, :, :], scale_factor=scale, mode="nearest")[0, 0]
        printedNom, printedMax, printedMin = litho(mask_bin)
        binaryNom = (printedNom >= 0.5).float()
        binaryMax = (printedMax >= 0.5).float()
        binaryMin = (printedMin >= 0.5).float()
        l2 = func.mse_loss(binaryNom, target, reduction="sum").item()
        pvb = torch.sum(binaryMax != binaryMin).item()
        # EPE和Shot可用printedNom/binaryNom继续算
        vposes, hposes = boundaries(target)
        epeIn, epeOut, _ = epecheck(binaryNom, target, vposes, hposes)
        epe = epeIn + epeOut
        nshot = -1
        # nshot = ShotCounter(litho, 0.5).run(mask, shape=(512, 512)) if shots else -1
    if verbose:
